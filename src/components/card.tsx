...
const Card = () => {
  ...
  return (
    <div className='flex flex-col items-center mt-10'>
      {jobs.map((item) => (
        <div key={item._id} className='grid grid-cols-12 w-full max-w-screen-xl mb-10 gap-6 justify-center items-center'>
          <div className='flex flex-col col-span-7 gap-5'>
            <div className='flex gap-5'>
              <img src={item.image} className='w-[89px] h-[89px] rounded-full' alt='Blog post' />
              <div className='flex flex-col gap-1'>
                {item.author ? (
                  <>
                    <p className='font-medium text-[24.43px]'>{item.author.name}</p>
                    <p className='font-medium text-[17.1px] text-[#9C9C9C]'>Software Campany</p>
                  </>
                ) : (
            ...
            <div className='flex gap-5'>
              {item.tags.map((tag, index) => (
                <button key={index} className='bg-[#737373] text-gray-700 rounded-[100px] p-4'>{tag}</button>
              ))}
            </div>
          </div>
          <div className='col-span-5 ml-6'>
            <img src={item.image} className='w-[450px] h-[295px]' alt='Blog post' />
          </div>
        </div>
      ))}
    </div>
